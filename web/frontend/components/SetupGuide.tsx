...
export const SetupGuide = (props: {
  onDismiss: any;
  onStepComplete: any;
  items: any;
}) => {
  const { items, onStepComplete, onDismiss } = props;
        ...
        <Box padding="400" paddingBlockEnd="400">
          <BlockStack>
            ...
            <Text as="p" variant="bodyMd">
              Complete the following steps to get your widget up and running.
            </Text>
...
const SetupItem = ({
  ...
  return (
    <Box
      borderRadius="200"
      background={(expanded && "bg-surface-active") || "bg-fill-active"}
    >
      <div
        className={`py-1 px-2 rounded-lg hover:bg-gray-100 ${expanded ? "bg-inherit" : ""}`}
      >
        <InlineStack gap="200" align="start" blockAlign="start" wrap={false}>
          ...
          >
            <Button onClick={completeItem} variant="monochromePlain">
              {
                (
                  <div className="flex justify-center items-center w-6 h-6 text-gray-800">
                    {loading ? (
                      <Spinner size="small" />
                    ) : complete ? (
                      <CheckIcon
          ...
          >
            ...
            {image.url && expanded ? (
              <Image
                className={
                  "flex gap-2 justify-between sm:hidden md:flex lg:flex"
                }
                source={image.url}
                alt={image.alt}
